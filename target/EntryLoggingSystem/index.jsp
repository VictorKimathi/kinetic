...
<%
    ...
    if ("students".equals(filterType)) {
        for (Map<String, Object> log : allLogs) {
            if ("student".equals(log.get("type"))) {
                filteredLogs.add(log);
            }
        }
    } else if ("visitors".equals(filterType)) {
        for (Map<String, Object> log : allLogs) {
            if ("visitor".equals(log.get("type"))) {
                filteredLogs.add(log);
            }
        }
    } else {
    ...
    // Get only recent 10 for dashboard display
...
<head>
    ...
    <title>Egerton University - Entry Management System</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .footer-btn { text-align: center; margin-top: 12px; padding-top: 8px; border-top: 1px solid #eee; }
        ...
        .sidebar::-webkit-scrollbar-track { background: #008f45; }
        .sidebar::-webkit-scrollbar-thumb { background: #d2ac67; border-radius: 5px; }
    </style>
</head>
<body>
    <div class="header">
        <div class="header-content">
            ...
            <div class="logo">
                <img src="logo.png?v=5" alt="Egerton University Logo">
                <div class="logo-text">
    ...
    <div class="layout">
        <div class="sidebar" id="sidebar">
            <div class="sidebar-menu">
                <a href="index.jsp?page=dashboard&filter=all" class="menu-item <%= "dashboard".equals(pageParam) ? "active" : "" %>">
                    <i class="fas fa-tachometer-alt"></i> Dashboard
                </a>
                <a href="index.jsp?page=student" class="menu-item <%= "student".equals(pageParam) ? "active" : "" %>">
                    <i class="fas fa-user-graduate"></i> Student Entry
                </a>
                <a href="index.jsp?page=visitor" class="menu-item <%= "visitor".equals(pageParam) ? "active" : "" %>">
                    <i class="fas fa-user-friends"></i> Visitor Entry
                </a>
                <a href="index.jsp?page=exit" class="menu-item <%= "exit".equals(pageParam) ? "active" : "" %>">
                    <i class="fas fa-sign-out-alt"></i> Exit Campus
                </a>
                <a href="index.jsp?page=logs&filter=all" class="menu-item <%= "logs".equals(pageParam) ? "active" : "" %>">
            ...
            <% if(overdueVisitors != null && !overdueVisitors.isEmpty()) { %>
                <div class="alert-card">
                    ...
                    <div class="alert-content">
                        <strong>⚠️ Overdue Visitors Alert!</strong><br>
            ...
            <div class="page <%= "dashboard".equals(pageParam) ? "active" : "" %>">
                <% if(personId != null) { %>
                    <div class="active-user-card">
                        ...
                        <a href="index.jsp?page=exit" class="btn btn-exit" style="display: inline-block; margin-top: 8px;">
                            <i class="fas fa-sign-out-alt"></i> Exit Campus
                        </a>
                    </div>
                <% } %>
                ...
                <div class="stats-grid">
                    <a href="index.jsp?page=dashboard&filter=all" class="stat-card <%= "all".equals(filterType) ? "active-filter" : "" %>">
                        <div class="stat-number"><%= stats.get("total") %></div>
                        <div class="stat-label">Total Entries</div>
                    </a>
                    <a href="index.jsp?page=dashboard&filter=students" class="stat-card <%= "students".equals(filterType) ? "active-filter" : "" %>">
                        <div class="stat-number"><%= stats.get("students") %></div>
                        <div class="stat-label">Students Today</div>
                    </a>
                    <a href="index.jsp?page=dashboard&filter=visitors" class="stat-card <%= "visitors".equals(filterType) ? "active-filter" : "" %>">
                        <div class="stat-number"><%= stats.get("visitors") %></div>
                        <div class="stat-label">Visitors Today</div>
                    </a>
                    <a href="index.jsp?page=logs" class="stat-card">
                ...
                <% if(!"all".equals(filterType)) { %>
                    <div class="filter-info">
                        ...
                        <a href="index.jsp?page=dashboard&filter=all" class="clear-filter"><i class="fas fa-times"></i> Clear Filter</a>
                    </div>
                <% } %>
                
                <!-- Currently Inside Campus Card -->
                <div class="card">
                    <div class="card-header">
                        <h2><i class="fas fa-users"></i> Currently Inside Campus</h2>
                        <% if(activeSessions.size() > 10) { %>
                            <a href="index.jsp?page=logs" class="view-all-btn"><i class="fas fa-eye"></i> View All (<%= activeSessions.size() %>)</a>
                        <% } %>
                    </div>
                            ...
                            <table>
                                ...
                                <tbody>
                                    <% for(Map<String, Object> s : recentActive) { 
                                        String statusBadge = "";
                                        String identifier = "";
                                        if("student".equals(s.get("type"))) {
                                            identifier = (String) s.get("reg");
                                            statusBadge = "<span class='badge badge-student'>Student</span>";
                                        } else {
                                            identifier = (String) s.get("name");
                                            boolean isOverdue = (Boolean) s.get("is_overdue");
                                            statusBadge = isOverdue ? "<span class='badge badge-overdue'>Overdue</span>" : "<span class='badge badge-visitor'>Visitor</span>";
                                        }
                                    %>
                                        <tr>
                                            ...
                                            <td><%= statusBadge %></td>
                                            <td><button class="view-details-btn" onclick="showDetails('<%= s.get("type") %>', '<%= s.get("name") %>', '<%= s.get("reg") != null ? s.get("reg") : "" %>', '<%= s.get("course") != null ? s.get("course") : "" %>', '<%= s.get("residence") != null ? s.get("residence") : "" %>', '<%= s.get("office") != null ? s.get("office") : "" %>', '<%= s.get("purpose") != null ? s.get("purpose") : "" %>', '<%= s.get("phone") != null ? s.get("phone") : "" %>', '<%= s.get("gate") %>', '<%= s.get("entry_time") %>', '', '', '<%= s.get("duration") %>', '', '')">View</button></td>
                                        </tr>
                                    <% } %>
                                </tbody>
                            </table>
                ...
                <!-- Recent Entry Logs Card -->
                <div class="card">
                    <div class="card-header">
                        <h2><i class="fas fa-clock"></i> Recent Entry Logs <%= !"all".equals(filterType) ? "(" + (filterType.equals("students") ? "Students" : "Visitors") + " Only)" : "" %></h2>
                        <% if(allLogs.size() > 10) { %>
                            <a href="index.jsp?page=logs&filter=<%= filterType %>" class="view-all-btn"><i class="fas fa-eye"></i> View All (<%= filteredLogs.size() %>)</a>
                        <% } %>
                    </div>
                    <div class="logs-table">
                        <table>
                            ...
                            <tbody>
                                ...
                                %>
                                    <tr>
                                        ...
                                        <td><%= durationStr %></td>
                                        <td><button class="view-details-btn" onclick="showDetails('<%= log.get("type") %>', '<%= log.get("name") %>', '<%= log.get("reg") != null ? log.get("reg") : "" %>', '<%= log.get("course") != null ? log.get("course") : "" %>', '<%= log.get("residence") != null ? log.get("residence") : "" %>', '<%= log.get("office") != null ? log.get("office") : "" %>', '<%= log.get("purpose") != null ? log.get("purpose") : "" %>', '<%= log.get("phone") != null ? log.get("phone") : "" %>', '<%= log.get("entry_gate") %>', '<%= log.get("entry_time") %>', '<%= log.get("exit_gate") != null ? log.get("exit_gate") : "" %>', '<%= log.get("exit_time") != null ? log.get("exit_time") : "" %>', '<%= durationStr %>', '<%= log.get("reason") != null ? log.get("reason") : "" %>', '<%= log.get("expected") != null ? log.get("expected") : "" %>')">View</button></td>
                                    </tr>
                                <% } %>
                                <% if(recentLogs.isEmpty()) { %>
                                    <tr><td colspan="8" style="text-align:center;">No logs found for this filter</td></tr>
                                <% } %>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="card">
                    ...
                    <div id="studentExitForm">
                        <form action="LoggingServlet" method="post">
                            ...
                            <div class="form-group">
                                <label><i class="fas fa-id-card"></i> Registration Number</label>
                                <input type="text" name="regNumber" placeholder="Enter your registration number" required>
                            </div>
                            <div class="form-group">
            ...
            <div class="page <%= "logs".equals(pageParam) ? "active" : "" %>">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="index.jsp?page=dashboard&filter=all" class="view-all-btn"><i class="fas fa-arrow-left"></i> Back</a>
                    </div>
                    <div class="logs-table">
                        <table>
                            <thead>
                            ...
                            <tbody>
                                ...
                                <% } %>
                                <% if(allLogs.isEmpty()) { %>
                                    <tr><td colspan="9" style="text-align:center;">No logs yet</td></tr>
                                <% } %>
                            </tbody>
                        </table>
                    </div>
    ...
    <script>
        ...
        function showDetails(type, name, reg, course, residence, office, purpose, phone, entryGate, entryTime, exitGate, exitTime, duration, reason, expected) {
            var modal = document.getElementById('detailsModal');
            var body = document.getElementById('modalBody');
            
            var html = '<div class="detail-row"><div class="detail-label">Type:</div><div class="detail-value">' + type + '</div></div>';
            html += '<div class="detail-row"><div class="detail-label">Name:</div><div class="detail-value">' + name + '</div></div>';
            
            if(type === 'student') {
                html += '<div class="detail-row"><div class="detail-label">Reg Number:</div><div class="detail-value">' + reg + '</div></div>';
                html += '<div class="detail-row"><div class="detail-label">Course:</div><div class="detail-value">' + course + '</div></div>';
                html += '<div class="detail-row"><div class="detail-label">Residence:</div><div class="detail-value">' + residence + '</div></div>';
                html += '<div class="detail-row"><div class="detail-label">Phone:</div><div class="detail-value">' + phone + '</div></div>';
            } else {
                html += '<div class="detail-row"><div class="detail-label">Office Visiting:</div><div class="detail-value">' + office + '</div></div>';
                html += '<div class="detail-row"><div class="detail-label">Purpose:</div><div class="detail-value">' + purpose + '</div></div>';
                html += '<div class="detail-row"><div class="detail-label">Phone:</div><div class="detail-value">' + phone + '</div></div>';
                if(expected && expected !== '') {
                    html += '<div class="detail-row"><div class="detail-label">Expected Duration:</div><div class="detail-value">' + expected + ' min</div></div>';
                }
            }
            
            html += '<div class="detail-row"><div class="detail-label">Entry Gate:</div><div class="detail-value">' + entryGate + '</div></div>';
            html += '<div class="detail-row"><div class="detail-label">Entry Time:</div><div class="detail-value">' + entryTime + '</div></div>';
            
            if(exitGate && exitGate !== '') {
                html += '<div class="detail-row"><div class="detail-label">Exit Gate:</div><div class="detail-value">' + exitGate + '</div></div>';
                html += '<div class="detail-row"><div class="detail-label">Exit Time:</div><div class="detail-value">' + exitTime + '</div></div>';
            }
            
            if(duration && duration !== '-') {
                html += '<div class="detail-row"><div class="detail-label">Duration:</div><div class="detail-value">' + duration + '</div></div>';
            }
            
            if(reason && reason !== '') {
                html += '<div class="detail-row"><div class="detail-label">Reason for Leaving:</div><div class="detail-value">' + reason + '</div></div>';
            }
